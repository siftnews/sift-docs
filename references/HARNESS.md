...
> **현재 상태 (D-039, 2026-08-06): 이 문서는 활성 하네스 사용법이 아니라 과거 운영 모델과 향후 재구축 참고 기록이다.**
> 현재 워크스페이스에는 `sift-harness`, 역할 런처, 권한 프로파일, bootstrap이 없다. 원격 환경이 필요해질 때 새 요구사항으로 재설계한다.
> 최종 수정: 2026-08-06
...
## 0.8 역할 체계 — 세션 3종 (과거 운영 모델, 현재 비활성)

> 아래 역할·런처·권한 프로파일은 D-034 당시의 운영 기록이다. 현재 세션 기동 절차로 사용하지 않는다.
...
| **하는 일** | 상태 파악 · 태스크 선정 · **읽기 전용 분석 병렬화·종합** · 이슈 발행 · 루프 문서 갱신 · 결정 정리 | 브랜치 · 코드 · 자가검증 · 커밋 · push · PR · 리뷰 반영 | PR 리뷰 · 외부 리뷰 공백 탐지 · 지적 유효성 판정 |
...
| 🧭 조율 | **기본으로 쓴다** | 영향 범위 탐색·설계·테스트·보안·문서 정합성의 읽기 전용 병렬 분석 |
...
| 요소 | 분류 | Sift에서의 모습 | 상태 |
|---|---|---|---|
| 규칙 주입 | — | 과거 루트 `CLAUDE.md`·`AGENTS.md` | 역사 기록 |
| 권한·훅 | — | 과거 `.claude/settings.json`과 역할 프로파일 | 역사 기록 |
| 역할 경계 | 워크플로우 | 과거 `sift-orch`·`sift-impl`·`sift-review` | 현재 비활성 |
| 반복작업 스킬 | 워크플로우 | "유스케이스 풀구현", "소스 어댑터 추가", "배치 Job 추가", "토픽 등록" | Phase 1 |
| 측정 골격 | — | Actuator 메트릭 + 배치 처리량·소요시간 로깅 (evals 토대) | **Phase 0~1 (앞당김)** |
| 역할 서브에이전트 | 에이전트 | 탐색·설계·테스트·보안·성능·문서 정합성 분석 | ✅ 운영 규칙 추가 |
...
- 하네스 토대: 원격 환경 구성 시 새로 결정한다 (현재 없음)
