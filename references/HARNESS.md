...
**한 사이클** (앞의 아이콘 = 그 단계를 맡는 역할 — §0.8)

1. 🧭 **상태 읽기** — `sift-docs/STATE.md` + 메모리로 현재 Phase·진행·다음 액션 파악 (**`sift-docs` fetch 먼저**)
2. 🧭 **태스크 선택** — `sift-docs/BACKLOG.md` 우선순위 최상단의 미완 작업 1개. DoD·의존성 확인 → 이슈 발행
3. 🔧 **실행** — 분류(§0.5)에 따라 워크플로우(스킬/고정 절차) 또는 에이전트(분석/탐색)
4. 🔧 **검증** — 컴파일·`build`·`test`는 에이전트가 자가검증. 되돌리기 어려운 명령·배포는 게이트(사람)
5. 🔧 **커밋** — 작은 작업 1개 완료·검증 통과 시 커밋 → 다음 작업 (D-026 — 형식·트레일러는 §0.7 컨벤션 자율 준수, 검사 장치 없음 D-027)
6. 👀 **검수** — PR이 열리면 독립 세션이 리뷰하고 리포트를 `.handoff/reviews/`로 (반영은 🔧, 결정은 👤)
7. 🧭 **기록·갱신** — STATE 갱신, BACKLOG 체크, 결정 났으면 DECISIONS 추가, 필요시 메모리

> 세 역할을 동시에 띄울 필요는 없다. 혼자 도는 사이클이면 `sift-orch` → `sift-impl` → `sift-review` 순으로 **바꿔 타는 것만으로도** 컨텍스트 격리와 권한 경계는 성립한다.
...
## 0.8 역할 체계 — 세션 3종 (D-034 · D-016 개정)

> 세션은 역할별로 나뉘고, **경계는 권한으로 강제된다.** 각 역할의 상세 절차는 별도 문서가 원본이다.
...
1. **모든 세션은 `siftnews/` 루트에서 실행한다.** 프로젝트 메모리는 실행 위치(cwd) 기준으로 잡히므로, 실행 위치가 다르면 메모리가 갈라지고 게이트(settings.json deny) 적용도 어긋난다. 런처 3종이 `cd`를 대신 해 준다.
2. **STATE·BACKLOG·TASKS·DECISIONS의 쓰기 주인은 🧭 조율 세션 1개.** 잠금 없는 단일 진실원천이라 동시 편집 시 나중 쓰기가 먼저 쓰기를 덮는다. 이 경계는 권한 프로파일로 강제된다 — 다른 역할은 규칙을 기억할 필요 없이 **막힌다**.
3. **같은 코드 파일을 두 세션이 동시에 편집하지 않는다.** 구현 세션이 2개 이상 필요해지면 탭 분리가 아니라 **git worktree**로 작업 트리를 분리한다.
4. **인계는 GitHub이 매체다.** 이슈와 PR이 이미 작업 큐이므로 별도 프로토콜을 만들지 않는다(원칙 6). 예외는 GitHub에 남길 수 없는 것뿐 — `gh pr review`·`comment`가 사람 영역(D-026)이라 검수 리포트는 파일로 나간다.
5. **이슈 1개 = 구현 세션 1개.** 이슈가 끝나면 세션을 버린다. 세션을 이어 쓰면 이전 이슈의 가정(브랜치·설계 판단·읽은 코드 상태)이 다음 이슈로 샌다 — 2026-08-04에 삭제된 브랜치를 체크아웃한 채 문서가 코드와 어긋났다고 오진할 뻔한 것이 그 사례다. 조율·검수 세션은 더 짧다(태스크 1건, PR 1건).
6. **세션 내 병렬화(서브에이전트·`/team`)는 역할 경계를 우회하지 못한다 — 실증됨.** 서브에이전트는 부모 세션의 deny를 그대로 상속한다(검수 프로파일에서 서브에이전트로 `sift-api` 편집을 시도해 `denied by your permission settings`로 차단 확인, 2026-08-04). 그래서 **경계는 공짜로 상속되지만, 단일 세션 + 서브에이전트로 역할 분리를 대체할 수는 없다** — 부모가 코드를 못 쓰면 자식도 못 쓰고, 부모 권한을 풀면 경계 자체가 사라진다.

**병렬화를 어디에 쓰는가** (규칙 6의 적용)

| 역할 | `/team`·서브에이전트 | 이유 |
|---|---|---|
| 👀 검수 | **기본으로 쓴다** | 설계 이탈·회귀 위험·문서 정합·CodeRabbit 지적 판정을 동시에 보는 것은 fan-out에 맞고, 전부 읽기 전용이라 충돌이 원천적으로 없다 |
| 🔧 구현 · 기능 | **쓰지 않는다** | 한 이슈의 변경은 도메인→포트→서비스→어댑터→테스트로 **의존 체인**이라(병합 PR 중앙값 13파일·최대 35파일) 워커를 늘려도 대기만 는다. TDD 순서 자체가 직렬이고, 같은 워킹 트리·같은 브랜치를 공유해 커밋 리듬이 깨진다 |
| 🔧 구현 · 기계적 반복 | **쓴다** | 링크 일괄 치환, 여러 파일에 같은 패턴 적용처럼 **의존 없는** 작업. 이건 진짜 fan-out이다 |
| 🧭 조율 | 가끔 | `gh` 다건 조회 등 |

> 워커마다 브랜치를 갈라 **진짜 병렬 구현**을 하려면 git worktree가 필요하다(규칙 3). OMC `/team`의 worktree 모드가 그것을 자동화하지만 opt-in이고, 우리 워크스페이스 루트가 git이 아니라 동작 여부는 **아직 확인하지 않았다.** 1인 사이클에서 이슈 2개 동시 구현은 리뷰 단위를 헝클어뜨리므로 필요가 실측되면 그때 검증한다.

**인계함 `.handoff/`** (워크스페이스 루트, git 없는 로컬 전용 — 수명이 짧은 세션 간 임시물)

| 경로 | 방향 | 내용 |
|---|---|---|
| `.handoff/reviews/pr-{N}-{날짜}.md` | 👀 → 🔧 | 검수 리포트. 반영 여부는 👤 결정 |
| `.handoff/notes/{이슈번호}-{요약}.md` | 🔧 → 🧭 | 루프 문서에 반영해야 할 것 (구현 세션은 `sift-docs`를 못 쓴다) |

처리한 인계물은 받은 쪽이 지운다 — 남아 있는 파일이 곧 미처리 목록이다.

**흐름 한 바퀴**

```
🧭 이슈 발행 ──▶ 🔧 브랜치·구현·커밋·PR ──▶ 👀 리뷰 → .handoff/reviews/
       ▲                    │                              │
       │                    ◀──────────────────────────────┘
       └── .handoff/notes/ ─┘                    (반영은 🔧, 결정은 👤)
                                   👤 병합 ──▶ 🧭 STATE·TASKS 갱신
```
...
| 역할 경계 | 워크플로우 | `.claude/roles/*.settings.json` 3종 + 런처 `sift-orch`·`sift-impl`·`sift-review` — 세션 역할을 권한으로 강제 (§0.8, D-034) | ✅ 완료 |
