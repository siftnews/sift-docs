...
> ※ D-023의 분할 트리거(결정 30+)에 도달했으나 **분할하지 않는다** — 171줄은 단일 문서로 관리되는 크기이고, "새 결정 맨 위" 규칙이 시간순 탐색을 제공한다. 파일을 쪼개면 원본이 늘어 D-023의 취지("사실마다 원본 1곳")와 어긋난다. **50개 또는 300줄에서 재검토** (2026-08-04 판단).
...
## D-036 · 외부 하네스 구조 참고 — 체크리스트만 흡수, 문서 분할·레포 재생성은 기각 (2026-08-04 · 두 논거 정정 → D-037)
