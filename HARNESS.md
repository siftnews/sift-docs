...
3. **컨텍스트는 외부화한다.** 결정·설계는 `docs/`와 메모리에 남겨 다음 세션이 cold start 하지 않게 한다.
...
**한 사이클**
1. **상태 읽기** — `docs/STATE.md` + 메모리로 현재 Phase·진행·다음 액션 파악
2. **태스크 선택** — `docs/BACKLOG.md` 우선순위 최상단의 미완 작업 1개. DoD·의존성 확인
...
- git/GitHub 쓰기 — **커밋·push·이슈 발행·PR 생성·병합 모두 사람 실행 (D-013·D-014, settings.json deny: git commit/push + gh issue/pr)**. 에이전트는 커밋 포인트에서 메시지를 제안하고 대기
...
> **이슈 1개 = 루프 1회분 = PR 1개.**
> **역할 분담 (D-013·D-014)**: git/GitHub 쓰기(**커밋**·이슈 발행·push·PR 생성·병합)는 👤 *사람*. 태스크 트리 구성·분해, 이슈/PR/**커밋 메시지** 초안, 브랜치·구현·자가검증은 🤖 *에이전트*.
> **main 직접 커밋 금지.**
...
- 이슈 제목 포맷: `[feature|chore|fix] 요약` (예: 이슈 #1 `[chore] 프로젝트 골격 & 모듈 경계`). 본문에 **배경 + DoD + 체크리스트** 필수.
...
- 위치 (D-021): **레포 종속 설계 문서·다이어그램**(ERD·이벤트·선별 등)은 해당 `sift-*` 레포의 `docs/`가 원본 — 구조 변경 PR에서 코드와 같은 diff로 리뷰한다.
  **워크스페이스 공통 문서**(PLAN·HARNESS·루프 문서)는 `sift-docs` 레포(= 루트 `docs/`)가 원본으로 공개된다. 스냅샷 사본은 만들지 않는다(drift 방지) — 크로스 레포 참조는 절대 URL로 링크한다.
