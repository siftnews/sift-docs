...
- [~] **#33 `[FEAT] collectionTrigger — 수집 배치 상시 기동`** — PR #34 리뷰 대기 (2026-07-31). `@Scheduled` 주기 기동 + `launchedAt` 식별 파라미터 + 스케줄러 풀 확보. **수집 주기 매시 10분 확정**(정각은 수집기가 몰린다). 브랜치 `feature/33-collection-trigger`, base=develop
...
- [ ] **🚨 `[FIX] 발행 이슈가 비는 시간대 결함 — runDate 존과 조회 하한 존 불일치`** — `BuildIssueService`가 `article_score` 조회 하한을 `runDate.atStartOfDay(ZoneOffset.UTC)`로 잡는데 **`runDate`는 시스템 존(KST) 날짜**다. KST 날짜 D의 UTC 자정은 KST D 09:00이라 **KST 00:00~09:00에 계산된 점수는 항상 하한 미만**이 되고, 확정 트리거 시각 **06:00 Asia/Seoul(#31)이 이 구간에 정확히 들어간다** → 운영에서 매일 빈 이슈가 발행된다. 재현: `TZ=Asia/Seoul`이면 `SelectionJobIntegrationTest` 2건 실패, `TZ=UTC`면 통과 — **CI가 UTC라 계속 녹색이었다**(#33 검증 중 develop에서 재현·확인). 수정 방향은 하한을 `runDate`와 같은 존의 자정으로 잡거나 트리거의 윈도우 `from`을 그대로 넘기는 것 — 두 존을 일치시키는 게 핵심
