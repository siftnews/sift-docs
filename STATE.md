...
## 현재 Phase
**Phase 1 (선별) 진행 중** — 골든패스 코드 경로는 M1-6에서 완주, M1-7 박제 태스크는 해체(D-029). M2-1·M2-2 병합으로 Phase 1 진입 완료

## 지금 (in progress) — 이슈 #21 착수 대기 (설계 확정, 구현 세션 담당)
- **#21 `[FEAT] 선별 normalizeDedup 재실행 멱등성` (M2-2 후속)** — PR #20 CodeRabbit Major 지적의 후속 분리 이슈. **범위·설계는 2026-07-25 확정 (D-031)**: fake 포트까지(실 어댑터 배선은 M2-5 유지) + `clusterId = "c-" + min(memberIds)`. 브랜치 미생성, BACKLOG 분해 완료 → 구현 세션이 착수
- ⚠️ **세션 역할 분담 (2026-07-25)**: 루프 문서(STATE·BACKLOG·TASKS·DECISIONS) 쓰기 창구 = **문서 세션**, 구현 세션은 `sift-api` 코드만 담당 (D-016 쓰기 주인 단일화)

## 다음 액션 (next)
- 🤖 **#21 구현** — 윈도우 로드 시그니처·상태 교체·clusterId 최소 id·벌크 갱신·순차 재실행 회귀 테스트 (BACKLOG 분해 참조)
- 🤖 **`[FEAT] 소스 RSS URL 확정 + source 시드` 발행** — TASKS M2 순서상 다음. **M1-6 e2e 게이트(👤)를 여기서 해소**
...
- (이전 완료 이력은 이 레포 git 히스토리로 조회 — D-023: STATE는 최근 4~5건만 유지. 하네스 개편 D-025·D-026, 가드 훅 폐지 D-027, edit 허용 D-028, M1-4 병합은 여기서 밀려남)
