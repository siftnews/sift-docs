...
> 프로토콜: [HARNESS.md §0.6](./HARNESS.md) · 마지막 갱신: 2026-07-16

## 현재 Phase
**Phase 0 — 스캐폴딩 + 골든패스** (진행 중)

## 지금 (in progress)
- **M1-4 `[FEAT] Source 영속 어댑터` PR #11 발행 완료, 리뷰·병합 대기** — 이슈 #10 → `feature/10-source-persistence-adapter` → PR #11 (2026-07-15). TDD로 구현, `./gradlew build` 전체 통과(Testcontainers 통합 테스트 포함). 👤 리뷰·병합 남음.

## 다음 액션 (next)
- 👤 **M1-4 PR #11 리뷰·병합** — 커밋·push·PR 발행 완료 (2026-07-15). ⚠️ gh 설정 커밋(3560400, D-024)이 같은 브랜치에 실려 PR #11에 포함됨 — 별도 커밋으로 분리는 되어 있음
- 🤖 **M1-5 `[FEAT] RSS 수집 어댑터 (RssFeedAdapter)` 착수** — M1-4 병합 후
...
- **M1-3 `[FEAT] CrawlSources 유스케이스` 병합 ✅ (이슈 #8 → PR #9 → develop, 2026-07-11)** — `CrawlSourcesUseCase`(crawlAll/crawl) + outbound port 3종 + `CrawlSourcesService`(소스 단위 실패 격리, Clock 주입). fake 포트 단위 테스트 통과. 리뷰 반영: `crawl(sourceId)` 단일 조회를 `LoadActiveSourcesPort.findActiveById`로 변경
- **문서 정합성 작업 (2026-07-11)** — MVP-DESIGN §4에 `findActiveById` 반영 + §6에 `markCrawled` 영속 반영 열린 질문 추가(M1-4 태깅), TASKS·BACKLOG·STATE 병합 상태 동기화
- (2026-07-11 이전 완료 이력은 이 레포 git 히스토리로 조회 — D-023: STATE는 최근 4~5건만 유지)
