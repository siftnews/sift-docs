...
## 문서 구조 개편 (2026-08-04 · D-036)

외부 팀 하네스 구조를 참고해 개편을 검토했다. **결론은 「체크리스트만 신설」**이다 — 대조해 보니 `references`·`adr`·`workflows`는 이미 있었고, 트리거 16개 중 6개는 Sift에 없는 기술이었다.

- ✅ **`checklists/` 신설** — [pr.md](./checklists/pr.md)(REQUIRED/OPTIONAL/**CONDITIONAL**) · [release.md](./checklists/release.md). 조건부 항목은 일반론이 아니라 **실제로 데인 사례**에서 뽑았다(#37 키 무효화 · #35 시각 의존 · D-029 유니크 제약 미소급 · #21·#39 재실행)
- ❌ **HARNESS·DECISIONS 분할 보류** — `§0.7`·`§0.8` 참조가 **33회·10개 파일**에 퍼진 관용구이고, 원본을 늘리면 D-023을 스스로 위반한다. DECISIONS는 분할 트리거(30+)에 닿았으나 171줄이라 **50개/300줄에서 재검토**
- ❌ **레포 삭제 후 재생성 기각** — 당시에는 sift-docs 커밋 이력 보존을 위해 기각했으나, 이 결정은 D-039로 폐기했다.

## 다음 액션 (next)
- ✅ **sift-api README 정정 완료 (2026-08-06)** — 현재 `sift-harness`가 없고 원격 환경에서 재구축한다는 상태를 반영했다.
- ⏸ **역할 런처·원격 bootstrap** — D-039에 따라 현재 중단. 원격 환경 구성 시 새 하네스 작업으로 재검토한다.
